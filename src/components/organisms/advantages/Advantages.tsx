...
const advantagesStatic = [
  ...
  {
    title: "600%",
    sub_title: "ROI от рекламы:",
  },
  {
    title: "80%",
    sub_title: "Средний рост заявок клиентов:",
  },
  {
    title: "1.5+",
    sub_title: "Количество просмотров органического контента в млн:",
  },
];
...
export const Advantages: React.FC<Smm> = ({ isSmm = false }) => {
  ...
  return (
    ...
    >
      <section className={styles.advantages__container}>
        <motion.div
          className={styles.advantages__text}
          variants={textVariants}
          style={isSmm ? { marginLeft: 0, maxWidth: "25rem" } : undefined}
        >
          <h3
            className={styles.advantages__title}
            style={isSmm ? { color: "#AAADB5", fontSize: "24px" } : undefined}
          >
            {isSmm ? "Цифры которые говорят сами за себя" : data?.title}
          </h3>
          <span
            style={isSmm ? { display: "none" } : undefined}
            className={styles.advantages__subtitle}
          >
            {data?.sub_title}
          </span>
        </motion.div>
        <motion.div
          ...
          variants={containerVariants}
        >
          <article className={styles.advantages__col1}>
            {advantages &&
              advantages.slice(0, 2).map((advantage, index) => (
                ...
                >
                  ...
                  <div
                    ...
                    style={
                      isSmm ? { flexDirection: "column-reverse" } : undefined
                    }
                  >
          ...
          <article className={styles.advantages__col2}>
            {advantages &&
